...
export const ImageMedia: React.FC<MediaProps> = (props) => {
  ...
  const loading: 'eager' | 'lazy' | undefined = loadingFromProps || (!priority ? 'lazy' : undefined)

  // ✅ For fill images, sizes should be a CSS length, not "w": use 100vw by default
  const sizes = sizeFromProps ?? (fill ? '100vw' : undefined)

  return (
    <picture
      className={cn(
        'relative block w-full h-full', // gives NextImage(fill) a positioned box
        pictureClassName,
      )}
    >
      <NextImage
        alt={alt || ''}
        className={cn('object-cover', imgClassName)} // avoid negative z-index here
        fill={fill}
